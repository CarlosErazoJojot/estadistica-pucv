...
with st.sidebar:
    ...
    st.page_link("pages/07_Distribuciones_Muestrales.py", label="Distribuciones Muestrales", icon=":material/arrow_back:")
    st.page_link("pages/09_Potencia.py", label="Potencia", icon=":material/arrow_forward:")
...
with tab3:
    ...
    # ── Dos proporciones ──────────────────────────────────────
        ...
        with c1:
            ...
            # Caso degenerado: p̄ = 0 o p̄ = 1 -> el error estándar es 0.
            # Ocurre cuando ambos grupos tienen 0 éxitos o ambos el 100%.
            degenerado = ee_dp == 0
            if degenerado:
                st.warning(
                    f"Con x₁ = {xg1} y x₂ = {xg2} la proporción combinada es "
                    f"p̄ = {p_pool:.0f}, así que el error estándar vale 0 y el "
                    "estadístico Z no está definido. Sin variabilidad en los datos "
                    "la prueba no puede aplicarse: ajusta los éxitos de algún grupo."
                )
                st.stop()
